...
if (array_key_exists('token', $_GET)) {
    $sql = "SELECT id, usuario FROM usuario WHERE token = ?";
    $st = $pdo->prepare($sql);
    $st->bindValue(1, $_GET['token']);
    $st->execute();
    if ($result=$st->fetch(PDO::FETCH_ASSOC)) {
        $sql = "UPDATE usuario SET token = null WHERE id = {$result['id']};";
        $pdo->exec($sql);
        ?>
        <h1>Bienvenido <?php echo $result['usuario'];?></h1>
        <form method="post">
    ...
    $sql = 'UPDATE usuario SET contraseña = '.password_hash($_POST['newPassword'], PASSWORD_BCRYPT).' WHERE id = '.$_POST['uid'];
    $pdo->exec($sql);
