...
if (array_key_exists('usuario', $_POST)) {
    try {
        ...
        $sql = "SELECT email FROM usuario WHERE usuario = ?";
        $st = $pdo->prepare($sql);
        ...
        if ($result = $st->fetch(PDO::FETCH_ASSOC)) {
            ...
            $sql = "UPDATE usuario SET token = '$token' WHERE email = '{$result['email']};'";
            ...
            try {
                ...
                try {
                    ...
                    //Recipients
                    $mail->setFrom('danielmaidana.prog@gmail.com', 'Daniel');

                    //Content
                    $mail->isHTML(true);                                  //Set email format to HTML
                    $mail->Subject = 'Recupere su clave';
                    $mail->Body    = 'Haga click en <a href="localhost/dev/recuperar.php?token='.$token.'">este link</a>';
